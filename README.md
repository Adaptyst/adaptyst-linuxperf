<!--
SPDX-FileCopyrightText: 2025 CERN
SPDX-License-Identifier: CC-BY-4.0
-->

# adaptyst-linuxperf

[![License: GNU GPL v2 only](https://img.shields.io/badge/license-GNU%20GPL%20v2%20only-blue)]()
[![Version](https://img.shields.io/github/v/release/adaptyst/adaptyst-linuxperf?include_prereleases&label=version)](https://github.com/Adaptyst/adaptyst-linuxperf/releases)

linuxperf is an Adaptyst system module currently built on top of Linux "perf" with custom patches. It analyses comprehensively on- and off-CPU activity of single- and multi-threaded programs.

## Disclaimer
This is currently a dev version and the module is under active development. Bugs are to be expected. Use at your own risk!

All feedback is welcome.

## License
Copyright (C) CERN.

The module is distributed under the GNU GPL v2 (**only this version**) license, with very limited exceptions. See the individual files for more detailed licensing information.

## Installation and documentation
All instructions related to adaptyst-linuxperf can be found [here](https://adaptyst.web.cern.ch/docs/modules/linuxperf-cpus-running-linux).

## Acknowledgements
The adaptyst-linuxperf development is possible thanks to the following funding sources:
* The European Union HE research and innovation programme, grant agreement No 101092877 ([SYCLOPS](https://syclops.org)).
* The European Union HE research and innovation programme, grant agreement No 101129744 ([EVERSE](https://everse.software)).
* The European Union HE research and innovation programme, grant agreement No 101070014 ([OpenWebSearch.eu](https://openwebsearch.eu)).
